...
void initConductivite(){
  int num  = 0; //numéro du port analogique
...
double getSale(float WaterTemp){
  int i;
  int tab[4];
  int num  = 0; //numéro du port analogique
  double  S,R,rt,Rt;
  char buffer[BUFFER_SIZE];
  int temperature,Capteur_conductivite;
  double Conductivite;
  ...
  printf("Pour une T=%d et une conductivitÃ© de :%f  on a la salinÃ© suivante  %lf \n",temperature,Conductivite,S);
  return S;
}
